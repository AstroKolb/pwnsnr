...
do k = 1, ks
 do j = 1, js
  do i = 1, imax
    ...
    if (zgm(i,j,k) < 4.9999 / 3.) then
       mfrac = (zga(i,j,k) + time*(4. - 3. * zgm(i,j,k))) / (5. - 3.*zgm(i,j,k))
       send_buff(i,4,j,k) = min(max(mfrac, 0.0), time)
    else
...
ncstat = nf90_def_dim(ncid, "y", jmax+1, yDimID); DimIDs(2) = yDimID
   if (ncstat /= nf90_NoErr ) print *, NF90_STRERROR(ncstat)
  ...
  do m = 1, npey
   do k = 1, ks
    do j = 1, js
     ...
     if (jsk == jmax) then
      do i = 1, imax
       prin_buff(i,jsk+1,k) = recv_buff(i,nv,1,k,1)
      enddo
     endif
    enddo
   enddo
...
ncstat = nf90_put_var(ncid, YScale_varID, tzyc)
