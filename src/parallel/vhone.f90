...
! LOCALS
  ...
  LOGICAL                                        :: prinENS, prinCDF

  INTEGER                                        :: i, j, k, n
  INTEGER                                        :: jcol, krow, nfile_start
  INTEGER                                        :: ncycend, nprin, ndump, nmovie
  INTEGER                                        :: tmpcyc, mpierr,start_cycl, namelength
  REAL                                           :: endtime,tdump,tprin,tmovie, olddt
  REAL(8)                                        :: start_time,end_time,run_time,zones

  INTEGER                                        :: ihead
  REAL                                           :: xexp, xexpand, yshift

! LISTS
NAMELIST / hinput / rstrt, format, ncycend, ndump, nprin, nmovie, endtime, tdump, tprin, tmovie

NAMELIST / icons  / prefix, profile, nssdw, Esn, Mej, d0, age, Lpsr0, v_psr, &
                    pbi, psrLT, x_amb, H_amb, z_amb,  psi_amb, seed

  REAL, DIMENSION(13)                            :: sendIC
...
! load cycle and print information
...
! set print formatting style
prinENS = .false.
prinCDF = .false.
if (format == 'ensight' .or. format == 'all') prinENS = .true.
if (format == 'netcdf'  .or. format == 'all') prinCDF = .true.



!--------------------------------------
! Setup dumpfile and initialize system
!--------------------------------------
...
if ((rstrt == 'NO').or.(rstrt == 'no')) then ! Initialize variables for new problem; 
 if (mype == 0) then
   ...
   write (8,*) 'History File for VH1-MPI simulation run on ', &
   ...
   call init
   nfile_start = nfile
   if (prinENS) call prin_ensight
   if (prinCDF) call prin
...
do while (ncycle < ncycend)
  ...
  if(mype == 0) write(*,*) 'STEP = ',ncycle, time / 3.15e7
...
!####################################################################
! Find the shock front and expand grid to contain shock

	do i = imax, 10, -1
	  if (zpr(i,1,1) > 1.e-11) goto 301
	enddo
...
!####################################################################
  ...
  call MPI_ALLREDUCE( xexpand, xexp, 1, VH1_DATATYPE, MPI_MAX, MPI_COMM_WORLD, mpierr )

  z_psr  = v_psr * time
  sdt    = 0.0
  dinflo = Lpsr0  * (1.0 + time / psrLT) ** ( (pbi + 1.) / (1. - pbi) )
  dinflo = dinflo * (1.0 - exp( - (time / 2. / psrLT)**4 ))
...
! Alternate sweeps to approximate 2nd order operator splitting
  ...
  sdt    = dt
  dinflo = Lpsr0  * (1.0 + (time+dt) / psrLT) ** ( (pbi + 1.) / (1. - pbi) )
  dinflo = dinflo * (1.0 - exp( - ((time+dt) / 2. / psrLT)**4 ))
...
!  endif
  ...
  if ( ncycd >= ndump .or. timed >= tdump ) then
    ncycd = 0
    timed = 0.0
    call dump(dmpfile)
  endif
