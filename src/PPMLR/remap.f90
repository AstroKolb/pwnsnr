...
! LOCALS
INTEGER :: n, nn
REAL, DIMENSION(maxsweep) :: du, ul, u6, dv, vl, v6, dw, wl, w6, de, el, e6, dg, gl, g6, dga, gal, ga6, dc, cl, c6
REAL, DIMENSION(maxsweep) :: dq, ql, q6, dr, rl, r6, dm, dm0, delta, dvol0
REAL, DIMENSION(maxsweep) :: fluxr, fluxu, fluxv, fluxw, fluxe, fluxq, fluxg, fluxga, fluxc
...
call parabola(nmin-1, nmax+1, para, g, dg, g6, gl, flat)
call parabola(nmin-1, nmax+1, para,ga,dga,ga6,gal, flat)
...
do n = nmin, nmax + 1
  ...
  if(deltx >= 0.0) then
    ...
    fluxg (n) = (gl(nn) + dg(nn) - fractn*(dg(nn) - fractn2*g6(nn)))*fluxr(n)
    fluxga(n) = (gal(nn)+dga(nn) - fractn*(dga(nn)-fractn2*ga6(nn)))*fluxr(n)
    ...
    fluxg(n) = (gl(n) - fractn*(dg(n) + fractn2*g6(n)))*fluxr(n)
    fluxga(n)= (gal(n)- fractn*(dga(n)+fractn2*ga6(n)))*fluxr(n)
...
do n = nmin-1, nmax+1  ! must update nmin-1, nmax+1 for possible second remap
  ...
  g  (n) = (g(n)*dm(n) + fluxg(n)-fluxg(n+1))*dm0(n)
  ga (n) = (ga(n)*dm(n)+fluxga(n)-fluxga(n+1))*dm0(n)
