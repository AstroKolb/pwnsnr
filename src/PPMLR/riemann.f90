...
REAL, DIMENSION(maxsweep) :: gamma, gamfac1, gamfac2
...
do l = lmin, lmax
  gamfac2(l) = gamma(l) + 1.0
  gamfac1(l) = 0.5*(gamfac2(l))/gamma(l)
  clft(l) = sqrt(gamma(l)*plft(l)*vlft(l))
  crgh(l) = sqrt(gamma(l)*prgh(l)*vrgh(l))
  vlft(l) = 1.0/vlft(l)
  vrgh(l) = 1.0/vrgh(l)
  plfti(l)= 1.0/plft(l)
  prghi(l)= 1.0/prgh(l)
...
do l = lmin, lmax
  do n = 1, 12
    pmold(l) = pmid(l)
    wlft (l) = 1.0 + gamfac1(l)*(pmid(l) - plft(l)) * plfti(l)   
    wrgh (l) = 1.0 + gamfac1(l)*(pmid(l) - prgh(l)) * prghi(l)
    wlft (l) = clft(l) * sqrt(wlft(l))      
    wrgh (l) = crgh(l) * sqrt(wrgh(l))   
    zlft (l) = 4.0 * vlft(l) * wlft(l) * wlft(l)      
    zrgh (l) = 4.0 * vrgh(l) * wrgh(l) * wrgh(l)   
    zlft (l) = -zlft(l) * wlft(l)/(zlft(l) - gamfac2(l)*(pmid(l) - plft(l)))      
    zrgh (l) =  zrgh(l) * wrgh(l)/(zrgh(l) - gamfac2(l)*(pmid(l) - prgh(l)))     
